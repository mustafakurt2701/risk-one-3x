...
async function runTick() {
  const store = getStore();
  if (store.tickPromise) {
    return store.tickPromise;
  }

  store.tickPromise = tick().finally(() => {
    store.tickPromise = null;
  });
...
function scheduleLoop(store) {
  ...
  store.timer = setInterval(() => {
    runTick().catch((error) => pushLog(store, `HATA ${error.message}`));
...
export async function startPaperTrader({ intervalSeconds = 10 } = {}) {
  ...
  if (store.running || store.starting) {
    ...
    if (!store.timer) {
      store.starting = true;
      persistStore(store);
      pushLog(store, "Motor yeniden baglaniyor.");
      await runTick();
